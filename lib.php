...
/**
 * Theme Boost Union Med2 Child - Library
 *
...
define('THEME_boost_union_med2_SETTING_INHERITANCE_INHERIT', 0);
define('THEME_boost_union_med2_SETTING_INHERITANCE_DUPLICATE', 1);
 ...
 * @return string
 */
function theme_boost_union_med2_get_main_scss_content($theme) {
    global $CFG;
    ...
    // This way, Boost Union Med2 Child will ship the same SCSS code as Boost Union itself.
    $scss = theme_boost_union_get_main_scss_content(theme_config::load('boost_union'));

    // And add Boost Union Med2 Child's main SCSS file to the stack.
    $scss .= file_get_contents($CFG->dirroot . '/theme/boost_union_med2/scss/post.scss');
 ...
 * @return string
 */
function theme_boost_union_med2_get_pre_scss($theme) {
    global $CFG;
    ...
    // If such a fault happens, the Boost Union Med2 Child administrator can switch the inheritance to 'Duplicate'.
    // This way, we will add the pre SCSS code with the explicit use of the Boost Union configuration to the stack.
    $inheritanceconfig = get_config('theme_boost_union_med2', 'prescssinheritance');
    if ($inheritanceconfig == THEME_boost_union_med2_SETTING_INHERITANCE_DUPLICATE) {
        $scss .= theme_boost_union_get_pre_scss(theme_config::load('boost_union'));
    }

    // And add Boost Union Med2 Child's pre SCSS file to the stack.
    $scss .= file_get_contents($CFG->dirroot . '/theme/boost_union_med2/scss/pre.scss');
 ...
 * @return string
 */
function theme_boost_union_med2_get_extra_scss($theme) {
    global $CFG;
    ...
    $inheritanceconfig = get_config('theme_boost_union_med2', 'extrascssinheritance');
    if ($inheritanceconfig == THEME_boost_union_med2_SETTING_INHERITANCE_DUPLICATE) {
...
/**
 * Callback function for theme_boost_union to allow Boost Union Med2 Child to add cards to the Boost Union settings overview page.
 * This function is expected to return an array of arrays containing values with the keys 'label', 'desc', 'btn' and 'url'.
 *
 * @return array
 */
function theme_boost_union_med2_extend_busettingsoverview() {

    $cards[] = [
        'label' => get_string('pluginname', 'theme_boost_union_med2'),
        'desc' => get_string('settingsoverview_buc_desc', 'theme_boost_union_med2'),
        'btn' => 'primary',
        'url' => new \moodle_url('/admin/settings.php', ['section' => 'theme_boost_union_med2']),
    ];
